...
function Chip({ children, tone = "stone" }: { children: React.ReactNode; tone?: string }) {
  // tone은 호출부 API 유지용 — 무채색 시스템에서 모든 톤은 동일한 중립 칩으로 렌더한다.
  const map: Record<string, string> = {
    stone: "border border-stone-700 bg-stone-800 text-stone-300",
    amber: "border border-stone-700 bg-stone-800 text-stone-300",
    emerald: "border border-stone-700 bg-stone-800 text-stone-300",
    blue: "border border-stone-700 bg-stone-800 text-stone-300",
  };
  return (
    <span className={`inline-block rounded-sm px-2 py-0.5 text-xs font-medium ${map[tone]}`}>
      {children}
    </span>
  );
}
  ...
  if (!unlocked) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-stone-950 px-4 text-stone-100">
        <form
          onSubmit={unlock}
          className="w-full max-w-md rounded-sm border border-stone-800 bg-stone-900 p-7"
        >
          <h1 className="text-2xl font-bold tracking-tight">노벨 바이블 백오피스</h1>
          <p className="mt-2 text-sm leading-relaxed text-stone-400">
            관리자 전용 화면입니다. 비밀번호를 입력해 주세요.
          </p>
          <div className="mt-5 rounded-sm border border-stone-700 bg-stone-800 px-4 py-3 text-sm text-stone-300">
            힌트: 관리자의 생일
          </div>
          <label className="mt-5 block">
            ...
            <input
              ...
              inputMode="numeric"
              autoFocus
              className={`w-full rounded-sm border bg-stone-950 px-3.5 py-3 text-base font-mono text-stone-100 outline-none transition ${
                authError ? "border-signal" : "border-stone-700 focus:border-stone-500"
              }`}
              placeholder="MMDD"
              value={password}
          ...
          {authError && <p className="mt-2 text-sm font-semibold text-signal">{authError}</p>}
          <button className="mt-5 w-full rounded-sm bg-stone-100 px-4 py-3 font-bold text-stone-900 transition hover:bg-stone-50">
            들어가기
          </button>
          <button
            type="button"
            className="mt-3 w-full rounded-sm border border-stone-700 bg-stone-800 px-4 py-2.5 text-sm font-semibold text-stone-300 transition hover:bg-stone-700"
            onClick={exitToSite}
          >
            사이트로 돌아가기
          </button>
  ...
  return (
    <div className="min-h-screen bg-stone-950 px-4 py-6 text-stone-100">
      <div className="mx-auto max-w-6xl space-y-5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stone-800 pb-4">
          <div>
            <h1 className="flex flex-wrap items-center gap-2 text-2xl font-bold tracking-tight">
              노벨 바이블 백오피스
              <span className="rounded-sm border border-stone-700 bg-stone-800 px-2 py-0.5 text-xs font-bold text-stone-300">
                비공개 · 링크 전용
              </span>
              {adminMode && (
                <span className="rounded-sm border border-stone-600 bg-stone-800 px-2 py-0.5 text-xs font-bold text-stone-100">
            ...
            <p className="mt-1 text-sm text-stone-400">
              ...
              봅니다. 데이터 출처:{" "}
              <b className="text-stone-100">{loading ? "불러오는 중…" : "전체 방문자 (중앙 서버)"}</b>
              {err && <span className="ml-1 text-signal">· {err}</span>}
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button className="rounded-sm bg-stone-800 px-3 py-2 text-sm font-semibold hover:bg-stone-700" onClick={refresh}>
              새로고침
            </button>
            <button
            ...
            <button className="rounded-sm bg-stone-800 px-3 py-2 text-sm font-semibold hover:bg-stone-700" onClick={exportCsv}>
              CSV 내보내기
            </button>
            <button className="rounded-sm bg-stone-100 px-3 py-2 text-sm font-bold text-stone-900 hover:bg-stone-50" onClick={exitToSite}>
              ← 사이트로
            </button>
          </div>
        </div>
        ...
        {/* KPI */}
        <div className="grid grid-cols-2 gap-px border border-stone-800 bg-stone-800 sm:grid-cols-4">
          {[
            ...
            <div key={k.label} className="bg-stone-950 px-4 py-4">
              <div className="text-xs font-semibold uppercase tracking-wider text-stone-500">{k.label}</div>
              <div className="mt-1 font-mono text-3xl font-bold text-stone-100">{k.value}</div>
            </div>
          ))}
        </div>

        {/* 역할 분포 */}
        <div className="rounded-sm border border-stone-800 bg-stone-900 p-5">
          ...
          {roleBreakdown.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2">
              {roleBreakdown.map((r) => (
                <span
                  key={r.role}
                  className="inline-flex items-center gap-2 rounded-sm border border-stone-700 bg-stone-800 px-3 py-1.5 text-sm font-semibold"
                >
                  {r.role}
                  <b className="font-mono text-stone-100">{r.count}</b>
                </span>
              ))}
            </div>
          )}
                ...
                <tbody>
                  {leads.slice(0, 8).map((lead) => (
                    <tr key={lead.id} className="border-b border-stone-800/70 align-top">
                      <td className="py-2.5 pr-3 font-mono text-stone-200">{lead.email}</td>
                      <td className="py-2.5 pr-3 font-mono text-stone-300">{shortKey(lead.sid)}</td>
                      ...
                      <td className="py-2.5 font-mono text-stone-400">{fmt(lead.ts)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="rounded-sm border border-stone-800 bg-stone-900 p-5">
          ...
          {visitors.length === 0 ? (
            <p className="rounded-sm bg-stone-800/60 p-4 text-sm text-stone-400">
                ...
                <tbody>
                  {visitors.map((v) => (
                    <tr key={v.sid} className="border-b border-stone-800/70 align-top">
                      <td className="py-2.5 pr-3 font-mono text-stone-300">{shortKey(v.sid)}</td>
                      <td className="py-2.5 pr-3 whitespace-nowrap">
                        {v.role ? v.role : <span className="text-stone-600">—</span>}
                      </td>
                      <td className="py-2.5 pr-3 whitespace-nowrap font-mono text-stone-400">{fmt(v.firstTs)}</td>
                      <td className="py-2.5 pr-3">
                      ...
                      <td className="py-2.5">
                        <button
                          className="rounded-sm border border-signal/50 bg-signal/10 px-2 py-1 text-xs font-semibold text-signal transition hover:bg-signal/20 disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <p className="mt-3 text-xs text-stone-500">
            ※ 삭제는 중앙 서버 기록에서 해당 방문 키의 이벤트를 제거합니다. 방문 키는 같은 브라우저와 같은
            사이트 주소에서 유지되며, 시크릿 모드·다른 브라우저·다른 기기에서는 새로 발급됩니다.
          </p>
        </div>
      </div>
    </div>
